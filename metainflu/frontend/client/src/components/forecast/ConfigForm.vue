    ...
    <div class="bg-white rounded-lg shadow-md p-8">
        <form @submit.prevent="onFormSubmit">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div>
                        ...
                        <select v-model="formData.country" class="input-field" required :disabled="loadingCountries">
                            <option value="" disabled>{{ loadingCountries ? 'Loading...' : 'Select country...' }}
                            </option>
                    ...
                    <div>
                        ...
                        <select v-model="formData.state" class="input-field"
                            :disabled="!formData.country || loadingSubdivisions">
                            <option value="">{{ stateSelectText }}</option>
                            <option v-for="sub in subdivisions" :key="sub.value" :value="sub.value">{{ sub.label }}
                            </option>
                        </select>
                    </div>
                    <div>
                        <label class="form-label">City</label>
                        <input type="text" v-model="formData.city" class="input-field"
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select v-model="formData.freq" class="input-field">
                            <option v-for="freq in FREQUENCIES" :key="freq.value" :value="freq.value">{{ freq.label }}
                            </option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" v-model="formData.horizon" class="input-field" min="1" max="365"
                            required />
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    <div class="flex items-center">
                        <input type="checkbox" id="apply_holidays" v-model="formData.apply_holidays"
                            class="h-4 w-4 rounded border-gray-300 text-soft-blue focus:ring-soft-blue">
                        <label for="apply_holidays" class="ml-2 form-label mb-0">Apply Country Holidays</label>
                    </div>
                    <div class="flex items-center">
                        <input type="checkbox" id="apply_ai_adjustment" v-model="formData.apply_ai_adjustment"
                            class="h-4 w-4 rounded border-gray-300 text-soft-blue focus:ring-soft-blue">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select v-model="formData.date_col" class="input-field" required>
                            ...
                            <option v-for="col in preview.columns" :key="col" :value="col">{{ col }} {{
                                detectedColumns.date?.name === col ? '(auto-detected)' : '' }}</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select v-model="formData.target_col" class="input-field" required>
                            ...
                            <option v-for="col in preview.columns" :key="col" :value="col">{{ col }} {{
                                detectedColumns.target?.name === col ? '(auto-detected)' : '' }}</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-end">
                <button type="submit" :disabled="isLoading" class="btn-primary flex items-center gap-2"
            ...
            <div class="mt-6 p-4 bg-light-blue rounded-md text-center">
                <p class="text-sm text-dark-slate-gray">
                    <strong>Privacy Notice:</strong> Your sales data is processed only on our server for modeling and
                    never sent to external AI models. Only anonymous context (industry/location/horizon) is sent to AI
                    models for macro adjustment.
                </p>
            </div>
        </form>
    </div>
...
const FREQUENCIES = [
    //{ value: 'D', label: 'Daily', description: 'Best for short-term planning' }, 
    { value: 'W', label: 'Weekly', description: 'Recommended for most businesses' }, 
    { value: 'M', label: 'Monthly', description: 'Good for long-term planning' }
];
